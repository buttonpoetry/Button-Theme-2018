...
if ( ! function_exists( 'bp_main_menu_append' ) ) {
	...
	function bp_main_menu_append ( $items, $args ) {
		...
		if ($args->theme_location == 'top-bar-r') {
			...
			ob_end_clean();
			$items .= '<li><a href="' . wc_get_cart_url() . '" class="nav-cart"><img class="desktop-cart-img" src="'. bp_mini_cart_src() . '"></a><ul class="submenu is-dropdown-submenu first-sub vertical dropdown menu">' . $bp_mini_cart . '</ul></li>';					
		}
	    return $items;
	}
}
